...
server <- function(input, output, session) {
  ...
  observeEvent(input$resetInputs, {
    ...
    updateCheckboxGroupInput(session, "sex", selected = c("Male", "Female"))
  })
  ...
  selectedGeneData <- reactive({
    ...
    dat <- pivot_longer(dat, cols = c(11:ncol(dat)),
                        values_to = "y",
                        names_to = "Gene")
  ...
  filterSummary <- reactive({
    ...
    sex <- format_list1(dat$sex)
    sex <- tolower(sex)

    # Construct final sentence
    glue::glue(
      "Based on your selection, the plot and statistics reflect data from myotubes grown from {sex} volunteers."
    )
  })
  ...
  statisticsData <- reactive({
    ...
    stats_result <- dat %>%
      group_by(Gene) %>%
      summarise(
        mean_Healthy = round(mean(y[condition == "Healthy"], na.rm = TRUE), 2),
        sd_Healthy = round(sd(y[condition == "Healthy"], na.rm = TRUE), 2),
        n_Healthy = sum(condition == "Healthy" & !is.na(y)),
        mean_T2D = round(mean(y[condition == "T2D"], na.rm = TRUE), 2),
        sd_T2D = round(sd(y[condition == "T2D"], na.rm = TRUE), 2),
  ...
  output$statistics1 <- DT::renderDT({
    dat <- statisticsData()
    dat <- dat[!dat$Statistics %in% c("mean Healthy", "sd Healthy", "n Healthy", "mean T2D", "sd T2D", "n T2D"),]
  ...
  output$downloadPlot <- downloadHandler(
    filename = function() {
      paste0("MyotubeT2D_plot_", Sys.Date(), ".png")
    },
  ...
  output$downloadData <- downloadHandler(
    filename = function() {
      paste0("MyotubeT2D_data_", Sys.Date(), ".csv")
    },
    content = function(file) {
      df <- selectedGeneData()
  ...
  output$downloadStats <- downloadHandler(
    filename = function() {
      paste0("MyotubeT2D_statistics_", Sys.Date(), ".csv")
    },
    content = function(file) {
      df <- statisticsData()
      if (!is.null(df)) {
