...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    p("For transcriptomics, raw UMI counts or CEL files were preprocessed with standard Bioconductor pipelines. Lowly expressed genes were filtered, normalized using TMM or RMA, and batch effects were removed with `limma::removeBatchEffect`. Gene identifiers were mapped to gene symbols. Metadata were extracted and harmonized to enable sample-wise comparison. Single muscle fibers were annotated based on the expression levels of MYH7, MYH1, and MYH2. Normalized expression values were back-transformed to linear scale and used to compute isoform contributions. Fibers were classified into 'Type I', 'Type IIA', or 'Type IIX' based on tertiles of isoform expression, with intermediate profiles labeled as 'Mixed'.")
